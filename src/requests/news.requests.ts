...
export const fetchNewsById = async (newsId: string): Promise<INewsData> => {
  const response: AxiosResponse<INewsData> = await API.get(
    API_URLS.NEWS.GET_BY_ID(newsId)
  );
  return response.data;
};
...
export const createNews = async (
  ...
  const response: AxiosResponse<INewsData> = await API.post(
    API_URLS.NEWS.CREATE,
    newsData
  );
  return response.data;
};
...
export const updateNews = async (
  ...
  const response: AxiosResponse<INewsData> = await API.put(
    API_URLS.NEWS.UPDATE(newsId),
    newsData
  );
  return response.data;
};
